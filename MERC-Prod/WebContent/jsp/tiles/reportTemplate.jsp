				...
				<table border = "0">
					...
					<tr class='<s:if test="#rowstatus.even">colored_td</s:if><s:else>white_td</s:else>' valign="top">
						...
						<s:a action="openReportTemplateReport1">
								 ...
								 <!--  <s:a id="reportWindow%{id}" onclick="reportWindow('/frameset?__report=%{reportFileName}&__parameterpage=false&userId=%{#session.CERC_USER.id}');" href="#"><s:property value="description"/></s:a>
							 <s:if test="isParameter==false">	
								<s:a id="reportWindow%{id}" onclick="reportWindow('/frameset?__report=%{reportFileName}&__parameterpage=false&userId=%{#session.CERC_USER.id}');" href="#"><s:property value="description"/></s:a>
							</s:if>
							<s:else>
								<s:a id="reportWindow%{id}" onclick="reportWindow('/frameset?__report=%{reportFileName}&__parameterpage=%{isParameter}');" href="#"><s:property value="description"/></s:a>
							</s:else> 	 -->
						</td>
					</tr>
					</s:iterator>	
