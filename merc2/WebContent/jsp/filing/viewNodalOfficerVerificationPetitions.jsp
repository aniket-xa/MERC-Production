...
<s:form theme="simple" name="registeredPetitionsCountList">
	<table border="0">
		<s:if test="hasActionMessages()">
			...
			<tr>
				<th colspan="7" class="mainheading">FILE MOVEMENT TRACKINGLIST</th>				
			</tr>
			
			<s:if test="filedPetitionsCountList.size > 0">
			<s:if test="%{#session.CERC_USER.refOrganisationId.id==1 and (officer=='pending'or officer=='register')and(#session.CERC_USER.userType == 'O' or (#session.CERC_USER.refDivisionId.divisionCode.trim()=='12' and  #session.CERC_USER.userType == 'E'))}">
			<tr>
			<td align="right" colspan="5">
				<s:submit type="submit" id="" name="" value="Registered" class="formbutton" action="viewPetitionsForpendingAdmin" />
				&nbsp;&nbsp;&nbsp;				
				<s:submit  type="submit" id="" name="" value="Pending" class="formbutton" action="viewPetitionsForApprovalAdmin"/>
				<%-- &nbsp;&nbsp;&nbsp;
				<s:submit  type="submit" id="" name="" value="Petitions for approval" class="formbutton" action="viewPetitionsForpendingAdminlist"/> --%>				
			</td>
			</tr>
			</s:if>
			...
			<tr>
				<td colspan="7">
					<display:table export="false" id="filedPetitions2" name="filedPetitionsCountList" pagesize="10" requestURI="">
						<display:column property="refPetitionTypeId.petitionTypeCode" title="Type" sortable="true" style="width:4%"/>
						<s:url var="url" action="filingNodalOfficerDetails0">
							<s:param name="struts.token.name">token</s:param>
							<s:param name="token" value="token" />
							<s:param name="filingBeanId" value="%{#attr.filedPetitions2.id}" />
						</s:url>
						<s:url action="viewPetitionUnderScrutinyHardCopyRecieved" var="url1" escapeAmp="false">
						<s:param name="struts.token.name">token</s:param>
						<s:param name="token" value="token"/>
                        <s:param name="filingBeanId1" value="%{#attr.filedPetitions2.id}"/>
        			</s:url>
<%--         		<s:if test="%{#attr.filedPetitions2.registrationNumberDisplay }== null"> --%>
        		
        		<display:column title="Petition Number" sortable="true" style="width:10%">
							<s:a href="%{url}">
							<s:property value="%{#attr.filedPetitions2.registrationNumberDisplay}"/>
...
<%--         		</s:if> --%>
						...
						<display:column title="Diary Number" sortable="true" style="width:8%">
							<s:a href="%{url}">
								<s:property value="%{#attr.filedPetitions2.diaryNumber}"/>/<s:property value="%{#attr.filedPetitions2.diaryYear}"/>
							</s:a>
						</display:column>
						...
						<s:property value="%{#attr.filedPetitions2.subject}"/>
						</display:column>
						<display:column title="Petitioner Name" sortable="true" style="width:15%">
							<s:property value="%{#attr.filedPetitions2.cercUserPetitionerId.userName}"/>
						...
						<display:column title="Filed By" sortable="true" style="width:15%">
							(<s:property value="%{#attr.filedPetitions2.cercUserFiledBy.userType}"/>)
							<s:property value="%{#attr.filedPetitions2.cercUserFiledBy.userName}"/>
						...
						<display:column  title="Status" sortable="true" style="width:10%">
							<s:property value="%{#attr.filedPetitions2.refStatusId.refStatusPublicDomainId.statusName}"/>
							<s:if test="%{#attr.filedPetitions2.refStatusId.isDivisionConcat==true}">
							    <s:iterator value="divisionList" status="divisionStatus">
							    	<s:if test='%{#attr.filedPetitions2.id == divisionList[#divisionStatus.index][0]}'>
                                		<s:property value="divisionList[#divisionStatus.index][1]"/>
                                	</s:if>
                                </s:iterator>
                            </s:if>
                        </display:column>
                        <s:if test="officer=='pending'">
                        <display:column property="position" title="Officer Level" sortable="true" style="width:8%"/>
                        </s:if>
			...
			<tr>
				<td colspan="7">
					<s:if test="%{#session.CERC_USER.refOrganisationId.id==1}">
						<s:a id="reportWindow" href="/merc/run?__report=ListOfFiledPetitions2.rptdesign&userType=cerc&userId=%{#session.CERC_USER.id}&__format=xls">
							<input type="button" id="printbutton" name="printbutton" value="Export To Excel" class="formbutton"/>
						</s:a>
					</s:if>
					<s:else>
						<s:a id="reportWindow" href="/merc/run?__report=ListOfFiledPetitions2.rptdesign&userType=other&userId=%{#session.CERC_USER.id}&__format=xls">
						<input type="button" id="printbutton" name="printbutton" value="Export To Excel" class="formbutton"/>
						</s:a>
					</s:else>
				</td>				
