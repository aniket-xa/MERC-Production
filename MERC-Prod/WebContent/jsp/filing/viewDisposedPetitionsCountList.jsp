			...
			<tr>
				<td colspan="7">
					<s:if test="%{#session.CERC_USER.refOrganisationId.id==1}">
						<s:a id="reportWindow" href="/run?__report=ListOfDisposedPetitions.rptdesign&userType=cerc&userId=%{#session.CERC_USER.id}&__format=xls">
							<input type="button" id="printbutton" name="printbutton" value="Export To Excel" class="formbutton"/>
						</s:a>
					</s:if>
					<s:else>
						<s:a id="reportWindow" href="/run?__report=ListOfDisposedPetitions.rptdesign&userType=other&userId=%{#session.CERC_USER.id}&__format=xls">
						<input type="button" id="printbutton" name="printbutton" value="Export To Excel" class="formbutton"/>
						</s:a>
					</s:else>
				</td>				
